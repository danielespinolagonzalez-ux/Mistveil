...
- [x] C1 Ampliar a ~50-60 reliquias (sobre todo JSON) que modulen ritmo/compás/SP, no solo +daño; algunas que escalen con desempeño rítmico (payoff de build) (carencias #5 y #10). **52 reliquias** (30→52): modulan cadencia/compás/SP/lágrimas/dash. Nueva mecánica mínima `groove_buff` (efecto) → 4+ reliquias cuyo bono SOLO rinde con la racha alta (payoff rítmico real). +3 sinergias. Todo data-driven con icono procedural.
...
- [x] C5 Activar los 6 sellos restantes con selector (cruza R2.5); rangos múltiples en el Santuario y un sumidero tardío para engranajes/Memoria. Los 8 sellos ACTIVOS con rasgos distintos (Vendaval/Peña/Alba/Ocaso/Trueno/Incoloro + Ascuas/Marea), cada uno con un `rasgo_fx` interpretado (stat_pasivo/armadura_sala/cura_sala/sp_por_perfecto/dash_burn/cura_por_sp) y finisher elemental. **Forja de Sellos** (pantalla desde el Santuario) = selector + sumidero de Memoria: forjar sellos, **subir su rango** (rangos múltiples que escalan rasgo y finisher) y equipar uno. Persistencia `selloRango`.
...
- [x] E5 Post-proceso barato en canvas (bloom en perfect/Ignición, viñeta con tinte por bioma) + toggle de screenshake/flash y `prefers-reduced-motion`; diferenciar el anillo de contraataque por forma/glifo, no solo color (carencia #15). **Hecho**: viñeta teñida por `bioma.tint`; bloom aditivo en cada golpe perfecto (`perfectGlowT`); `FX.shakeMult` desactiva sacudidas y `flashK` los fogonazos de pantalla; dos interruptores nuevos en Opciones (Sacudidas / Destellos de pantalla, con `UIK.toggle` etiquetado) que arrancan según `prefers-reduced-motion`; el anillo ROJO de contraataque lleva ahora 8 púas hacia dentro + glifo `◈ ¡E!` (legible con daltonismo).
...
- [x] F0 Cimientos: `data/ciudad.json` (11 distritos + `unlock`) + `js/ciudad.js` (3 terrazas, calle en arco, estado `ruina`/`viva` derivado); `GameState.rescatados` + `SaveManager` schema 4 con migración. Migrar los 7 NPCs actuales a datos. **Hecho**: `js/ciudad.js` absorbe `pueblo.js` (borrado); 11 distritos data-driven en 3 terrazas; `barrioVivo()` deriva ruina/viva de `unlock` (inicio/rescate/nivel/flag/muerte/rescatados_min); 5 vivos de inicio (Margo/Tablón/Gremio/Mirador/Puerta), 5 por rescate, Redoble por hito; `main.js` delega y solo los distritos VIVOS interactúan; decorado por código coherente (la lámina plana `fondo_pueblo` se retira hasta el LOTE CIUDAD de F4); `EventBus 'barrio_iluminado'` con fanfarria de encendido; schema 4 + migración tolerante. Smoke 219/0 (+7 asserts F0).
- [x] F1 Bucle de rescate: `data/rescates.json` + `type:'rescate'` en `rooms.js` (cerrojo rítmico) → `npc_rescatado`→`barrio_iluminado` + secuencia de encendido + `flash`. **Hecho**: `data/rescates.json` (5 cautivos con `barrio`/`pisos`/`guardias`/`cerrojo`); `type:'rescate'` en `rooms.js` (celda con cautivo + guardias `_spawnGuardias` + altar-cerrojo; `rescateElegible(piso)` + siembra en `buildTower`, `prob` en balance); QTE **EL CERROJO** (`mode:'cerrojo'`, reusa el latido del metrónomo: PARADA al compás, N/4 aciertos, fallar no castiga → accesible/móvil); al romperlo → `GameState.rescatados += id` (persistido) + recompensa de Memoria + `EventBus 'npc_rescatado'`; al volver al hub el distrito **se enciende** (fanfarria de F0). Cautivo dibujado (figura gris en jaula de engranajes que se difumina al limpiar la celda). Smoke 226/0 (+7 asserts F1). Loop verificado en-motor (celda→cerrojo→liberar→Contaduría viva).
- [x] F2 Profundidad y vida: parallax + oclusores en primer plano + vecinos ambientales que pasean por distritos vivos + niños/Tuerca al superar `rescatados_min`. **Hecho**: capa LEJANA con parallax (silueta del **Reloj colosal DETENIDO** + terrazas en la bruma, scroll 0.4); **vecinos** que pasean por cada distrito VIVO (color de su acento) + paseantes genéricos (más cuantos más rescatados → la villa bulle) + niños al ≥3 rescatados; **fuente-oclusor** de relojería en primer plano (Pip pasa por detrás, y-sort compartido); **motas** de polvo dorado. Todo por código en `ciudad.js`, verificado en escritorio y **móvil** (todo scrollea junto al mundo). Pendiente opcional: *pan* vertical al subir de terraza (la villa cabe en pantalla). Smoke 228/0.
- [x] F3 Servicios de los distritos rescatados. **Hecho**: la **Fragua** (Yelmo) abre la **Forja de Sellos** (movida desde el Santuario, que ahora la muestra "fría" hasta rescatar a Yelmo); la **Botica** (Nébula) vende el **ungüento** (bendición: +1 corazón + cura al descender); el **Conservatorio** (Sostenido) **afina los compases** (+usos de ensayo → suben de nivel antes); la **Atalaya** (Alondra) **revela el mapa** del próximo piso. Todos con Memoria como moneda (costes en `balance.ciudad_servicios`), vía diálogo+onDone (sin pantallas nuevas salvo la Forja existente). Smoke 233/0 (+5 asserts). Pendiente opcional: capa musical por distrito (A5), no crítica.
- [x] F4 Vestido de arte: lotes CIUDAD-1…6 de Recraft, uno por sesión, con visto bueno de Daniel (≈130 cr). **CIUDAD-1 hecho**: `fondo_ciudad` (backdrop pintado FFIX de Cuerdaqueda — ciudad-reloj en terrazas a hora dorada bajo el Reloj colosal detenido, sin gente) generado (n=2, 4 cr), troceado a JPEG 1024×512 e integrado anclado al mundo detrás de las estaciones/vecinos (fallback al código si falta); transforma la villa a nivel "mazmorra pero más currada". Verificado escritorio + móvil, smoke 233/0, bundle arranca. **CIUDAD-4 hecho**: 4 retratos de diálogo pintados (Yelmo→`retrato_fragua`, Nébula→`retrato_botica`, Sostenido→`retrato_conservatorio`, Alondra→`retrato_atalaya`) — n=2 c/u, elegidos, `remove_background`, troceados a busts ~119×160 con alfa; **drop-in cero código** (el sistema de diálogo ya busca `retrato_<servicio>`). Los 4 vecinos rescatables tienen cara en sus diálogos. Verificado en-motor (busto de Yelmo aparece), smoke 233/0, bundle arranca (~16 cr). **CIUDAD-3 hecho**: 6 sprites de cuerpo entero (los 4 rescatables COMO ELLOS MISMOS — Yelmo/Nébula/Sostenido/Alondra pasean su calle al rescatarlos — + 2 vecinos genéricos), n=2 c/u, `remove_background`, troceados ~30×60; `drawVecino` usa el sprite (volteo por dirección) con fallback al blob de código; `poblarVecinos` asigna el sprite pintado a cada rescatado. Los blobs de código pasan a personas pintadas, coherentes con el backdrop. Verificado escritorio + móvil, smoke 234/0, bundle arranca (~30 cr). **CIUDAD-5 hecho**: 3 props de primer plano pintados (fuente de relojería, puesto de mercado, cajas+barril), n=2 c/u, `remove_background`, `drawProp` con sombra de contacto + y-sort (Pip pasa por detrás), fallback por código solo la fuente. Smoke 234/0, ~15 cr. **CIUDAD-2 hecho**: 5 fachadas pintadas de los distritos rescatables (fragua/hermanos-Contaduría/botica/conservatorio/atalaya) — cada rescate enciende su edificio; `drawDistrito` pinta `fachada_<id>` anclada al suelo (escala 0.66) con el farol+emblema por delante, fallback natural para los 6 siempre-vivos; stack por terrazas da profundidad; atalaya regenerada para que contraste. Smoke 236/0 (+1 assert), ~32 cr. **CIUDAD-6 hecho**: cautivo pintado (jaula de engranajes con el vecino preso + candado-engranaje), neutro para teñir por código; `drawBiomaExtras` lo pinta anclado a la base con fallback al blob de código, y el estado `listo` (celda limpia) añade aura del color del distrito + destello del candado (telegrafía "rompe el cerrojo"). Smoke 237/0 (+1 assert), ~5 cr. **Fase F/CIUDAD COMPLETA**: Cuerdaqueda vestida entera (backdrop + retratos + vecinos + props + fachadas + cautivo).
